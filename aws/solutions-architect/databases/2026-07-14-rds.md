...
* AWS `Aurora` is a fully managed, high performance relation db compatible with both `mysql` and `postgresql`
    ...
    - you can enable `babelfish for aurora postgresql` which allows db to intepret commands from applicaions developed on `microsoft sql server`
        * enabling sql server applications to run directly on aurora postgresql with minimal or no changes required to the code
    - you can add replicas to the cluster with a differnt class (e.g adding a serverless instance class)
    - you can invoke a `Lambda function` from an `Amazon Aurora MySQL-Compatible Edition DB cluster` using a native function (`lambda_sync or lambda_async`)
    ...
    - you can create custom endpoints your app can use to redirect traffic to specific instances
