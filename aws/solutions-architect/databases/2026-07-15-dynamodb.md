...
- `on demand classes` are also included, where no capacity planning is neeeded
    * create for cost optimization as user base grows, enables the db to auto scale up or down to adjust for workload
...
4. `on demand backup and restores`: cannot backup to other regions or accounts
    * to copy to other region, use `aws backup` to copy recovery point, export to `s3`, import to other region
