...
* `Automated Backups`: backup to s3 and point in time recovery, cross region snapshots can be created
...
* `Spectrum`: allows you to run sql queries directly against data stored in s3 without needing to load or transform it
    - extends `dwh` into exabyte-scale datalke, and you pay only for bytes scanned during query rather than paying for idle compute time or storage
