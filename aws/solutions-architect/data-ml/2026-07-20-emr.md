...
* Amazon `Elastic MapReduce (emr)` is a managed cloud platform that simplifies running large-scaled distributed data frameworks, such as `apache spark` and `hadoop`, on aws
    ...
    - used for big data processing often in batch mode, its great for vast amounts of data but is not optimized for real time streaming data
...
* `EMR Studio`: is a web based integrated dev env (IDE) for fully managed jupyter notebooks that run on emr clusters