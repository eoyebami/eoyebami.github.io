...
* NOTE: `s3 lifecycle policies` manage object movement, archiving and deletion over time by defining when to shift inactive data to cheaper tiers and to cleanup outdated or expired assests automatically

* NOTE: `s3 storage class analysis` is a tool that monitors `object` access patterns to help you determine when to transition data to lower cost `storage classes`
...
* `s3 acls` are a legacy access control mech that predates `iam` and its very inflexible and has a limited set of rules. It allowed fine grained, object level permissions. It could be attached to individual files
...
* `S3 Transfer Acceleration` speeds up uploading and downloading files to and from `buckets` over long distances by routing data through the closest `cloudfront edge location` to bypass internet congestion
    - you hit the edge location and then get routed through aws' backbone (similar to `global accelerator`)
    - you enabled pere bucket and use the specific `transfer-accelerated` endpoint instead of the standard endpoints

### Replication

#### SRR

* AWS `S3 SRR` feature that auto (live replication) and asynchronously replicates new uploaded `objects` between buckets located in the same region
    - Requires versioning be enabled
    - You can be granular with what you replicate

#### CRR

* AWS `S3 CRR` feature is identical to `srr` but its across regions to
    - Greated for replicating backup data for DR scenerios

#### Encryption

* By default all `s3` applies server side encrption with either aws managed keys or `kms` keys
* Client side encryption can be perfomed with `AWS S3 Encryption Client`
* For encryption in transit, you can enforce this with a `bucket policy`
    ```json
    // enforce https 
    {
        "Version":"2012-10-17",
...
* When using `customer provided keys (SSE-C)` for encryption the following headers are required by `s3`
  - `x-amz-server-side-encryption-customer-algorithm`: specifies the encryption algo, required to be `AES256`
  - `x-amz-server-side-encryption-customer-key`: contains actual, 256-bit encryption key 
  - `x-amz-server-side-encryption-customer-key-MD5`: used by `s3` as a message integrity check to ensure ur keys are transmitted across without corruption or errors, must be a based64 128-bit md5 digest of your raw binary key
* For AWS managed keys (SSE-S3), only `x-amz-server-side-encryption` is needed
* More encryption info [here](https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingEncryptionInTransit.html)

### Things to Be Aware of

* [S3 Multipart Upload](https://docs.aws.amazon.com/AmazonS3/latest/userguide/mpuoverview.html) allows you to upload large files to s3 as smaller parts to improve upload speed, bandwidth through (smaller packets)

* [S3 Event Notifications](https://docs.aws.amazon.com/AmazonS3/latest/userguide/EventNotifications.html) allows you to triggered automated responses when an `object` is created, removed, or restored in a `bucket.
    - Destination can be `sqs`, `sns`, `lambda`

* `S3 Object Lock`:
    - `governance mode`: specific users with iam perms can overwrite or delete protected object versions during retention period
    - `compliance mode`: no user can overwrite or delete protected object versions during retention period
    - `legal hold`: prevents specific objects from being overwritten or deleted, independent ov retention period and must be explicitly remove; `WORM(write once read many)` protection