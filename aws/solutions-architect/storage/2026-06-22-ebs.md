...
* The way that `block storage` works is that it breaks up data into blocks and stores these blocks as separate pieces each with its own identifier. These blocks can then be storage across a variety of different physical devices
...
* Amazon's `block storage` service is known as `elastic block storage (ebs)`
    ...
    - What makes `ebs` so unique, is that they are abstract from `ec2 instances`
        * You can attach/dettach from instances
            ![alt text](images/ebs/image-5.png)
...
* NOTE: after attaching a volume, you'll need to create a `fs`, notes on that can be found [here](../../../linux/2023-05-17-fdisk.md)
...
* AWS allows you to use a feature known as `snapshots`
    - You take a `snapshot` of an `ebs` and create an `image` in `s3`
        * `S3` is available across all `AZs`
        1. ![alt text](images/ebs/image-6.png)
        2. ![alt text](images/ebs/image-7.png)
    - From this `snapshot` we can create a brand new `volume` in whatever `AZ` you'd like
        1. ![alt text](images/ebs/image-8.png)
        2. ![alt text](images/ebs/image-9.png)
    ![alt text](images/ebs/image-1.png)
...
* This method works across `AZs`, but how do we copy data across `regions`. 
    * Similarly to how we do it across `AZs`, we can take a `snapshot` and store it in `s3`, and move the data from the `s3` in the source region and copy it to an `s3` in the destination region
        ...
        1. ![alt text](images/ebs/image-10.png)
        2. ![alt text](images/ebs/image-11.png)
        3. ![alt text](images/ebs/image-12.png)
    * ![alt text](images/ebs/image.png)

* EBS suppports `ebs fast snapshot restore (fsr)` which enables you to create a volume from a `snapshot` that is fully initialized at creation 
    - eliminates the latency of i/o operations on a block when it is accessed for the first time
    - volumes restored this way deliver all their provisioned performance
    - regular snapshots use lazy loading which causes latency on first block reads
...
* ![alt text](images/ebs/image-3.png)
...
* ![alt text](images/ebs/image-2.png)
...
* ![alt text](images/ebs/image-4.png)
1. `Magnetic`:
