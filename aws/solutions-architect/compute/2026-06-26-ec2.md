...
* NOTE: `CloudWatch` has available Amazon EC2 Metrics for you to use for monitoring `CPU utilization`, `Network utilization`, `Disk performance`, and `Disk Reads/Writes`
...
* These templates support versioning as well as customization with defined parameters

#### How to

1. Create a template
    - ![alt text](images/asg/image.png)

2. Specify `ami`
    - ![alt text](images/asg/image-1.png)

3. Specify instance type
    - ![alt text](images/asg/image-2.png)

4. Specify key pair and networking
    - ![alt text](images/asg/image-3.png)

4. Specify storage and tags
    - ![alt text](images/asg/image-4.png)
...
* AWS `ec2` has a variety of different pricing options
    1. `On Demand`: allows you to pay for compute resources by hour or second with no long term commitments or upfront payments
        ...
        - there is a limit on running on demand instances per vpcu-based on demand limit
    ...
    4. `Reserved Instances`: same as `savings plan` but instead of committing to spending a certain amount per month, your committing to use a certain amount of compute power per month in exchange for discounted prices
        - capped at 20 per region depending on instance types
...
* `ec2` autoscaling ensures that you have the exact right number of instances running to handle your application traffic
    - it automatically scales out (adds more nodes) or scales in (removes nodes) to reduce costs, while also replacing unhealthy nodes
* You configure an `auto scaling group (asg)` which acts as a collection of instances with the same characteristics
    - these `asg` include
        * `launch templates`: blueprints for instances
        * `scaling policies`: rules that dictate when to scale
            - dynamic scaling: responds to real time metrics
                * target tracking: cpu utilization, mem utilization, etc, (e.g if utilization is above 50% for more than 5min, scale up)
                * simple scaling: uses cloudwatch alarms to scale, more flexibility because these alarms don't have to be related to the `ec2` and you can define by how much (fixed value) you want to scale (you can have one alert for increase and another for decreasing)
                * step scaling: just simple scaling, but you can tier based on these alerts (e.g over 60% utilization add 1 instance, if 80% add 3 instances)
            - predictive scaling: using ml to forecast demand
            - scheduled scaling: cron for scaling
        * `capacity limits`: you define desired capacity, minimum capacity, and maximum capacity
* You can configure `asg` with spot instances to create a mixed instance group
    - allows you to combine on-demand instances for a guaranteed baseline with spot instances
    - you can define the ratio of on-demand to spot
    - great for scaling with cost optimization
    - ![alt text](images/asg/image-13.png)
* NOTE: typically meant for stateless applications
* NOTE: when scaling in `asg` chooses based on priority below:
    1. `AZ` with the most instances
    2. instance with oldest launch template
    3. instance closest to next billing hour
    4. instance at random
* NOTE: `asg cooldown period`
    - ensures `asg` do not launch of terminate additional `ec2` before previous scaling activity takes effect
    - default value is 300s
    - is configurable in an `asg`

#### How to

1. Create `asg` and specify `launch template`
    - ![alt text](images/asg/image-5.png)

2. Specify networking
    - ![alt text](images/asg/image-6.png)

3. Specify template overrides
    - ![alt text](images/asg/image-7.png)

4. Attach lb and latticd
    - ![alt text](images/asg/image-8.png)

5. Configure health checks
    - ![alt text](images/asg/image-9.png)

6. Configure `asg` capacity
    - ![alt text](images/asg/image-10.png)

7. Define scaling policy
    - ![alt text](images/asg/image-11.png)
    * ![alt text](images/asg/image-12.png)
        - scale in protection can be added